...
def stop_condition() -> Generator[Any, Any, Any]:
    condition = TriggerMessageCondition()
    yield condition
...
def test_stop_exception_when_already_triggered(stop_condition: Any) -> None:
    ...
    with pytest.raises(ContextPlusException):
        asyncio.run(stop_condition([trigger_msg]))
...
# Test MaxMessageCondition
@pytest.fixture
def max_condition() -> Generator[Any, Any, Any]:
    condition = MaxMessageCondition(max_messages=2)
    yield condition
    asyncio.run(condition.reset())
...
def test_max_trigger_when_max_reached(max_condition: Any) -> None:
    ...
    result2 = asyncio.run(max_condition([msg1, msg2]))
...
# TextMentionCondition fixture
@pytest.fixture
def text_mention_condition() -> Generator[Any, Any, Any]:
    trigger_text = "trigger me"
    condition = TextMentionCondition(text=trigger_text)
    yield condition, trigger_text
    asyncio.run(condition.reset())


# Test TextMentionCondition
...
# Test TimeoutCondition
@pytest.fixture
def timeout_condition() -> Generator[Any, Any, Any]:
    condition = TimeoutCondition(timeout_seconds=0.2)  # Short timeout for testing
    yield condition
    asyncio.run(condition.reset())
...
# Test TextMessageCondition
@pytest.fixture
def text_message_condition() -> Generator[Any, Any, Any]:
    condition = TextMessageCondition(source="test_source")
    yield condition
    asyncio.run(condition.reset())
...
def test_text_message_any_source() -> None:
    condition = TextMessageCondition()  # No source specified, should match any
...
# Test FunctionCallCondition
@pytest.fixture
def function_call_condition() -> Generator[Any, Any, Any]:
    condition = FunctionCallCondition(function_name="test_function")
    yield condition
    asyncio.run(condition.reset())
...
# Test SourceMatchCondition
@pytest.fixture
def source_match_condition() -> Generator[Any, Any, Any]:
    condition = SourceMatchCondition(sources=["test_source", "another_source"])
    yield condition
    asyncio.run(condition.reset())
...
def test_and_condition() -> None:
    condition1 = MaxMessageCondition(max_messages=2)
    condition2 = TextMentionCondition(text="trigger me")
